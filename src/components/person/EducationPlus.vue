<template>
    <div class="flex">
        <div v-if="username" @click="showForm = !showForm" class="p-2 my-5 text-blue-600 border-2 border-blue-600 transition cursor-pointer rounded-md hover:text-white hover:bg-blue-600">Добавить дополнительное образование</div>
    </div>
    <form v-if="showForm" class="flex flex-col border-b border-blue-600 pb-10" @submit.prevent="writeToDb('createEducationPlus', doc.year, doc.text, props.id, username)">
